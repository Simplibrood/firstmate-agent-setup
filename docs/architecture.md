...
Once a `paused:` declaration's own stated `until` time has passed, the attended watcher poll also steers that crewmate back to work itself through `bin/fm-pause-resume.sh`, rather than only asking firstmate to come and steer by hand, and still reports every steer it sent or could not send.
That sweep is deliberately independent of the staleness backbone, because a worker whose harness keeps repainting its pane never holds two identical hashes and so never reaches that backbone at all; it is bound to one steer per declaration, excludes persistent secondmates, and never writes a worker's own status ledger.
