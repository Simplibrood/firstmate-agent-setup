...
Once a `paused:` declaration's own stated `until` time has passed, the watcher poll also steers that crewmate back to work itself through `bin/fm-pause-resume.sh`, rather than only asking firstmate to come and steer by hand, and still reports every steer it sent or could not send; the recheck above is unchanged and still fires on its own cadence beside that report.
That sweep is deliberately independent of the staleness backbone, because a worker whose harness keeps repainting its pane never holds two identical hashes and so never reaches that backbone at all; it is bound to one steer per declaration, paces a failed steer off the declared wait's own recheck cadence rather than a tighter floor, stands down for a positively gone endpoint after reporting it once for recovery, excludes persistent secondmates, is skipped entirely while either away marker exists because the away daemon owns that posture, and never writes a worker's own status ledger.
