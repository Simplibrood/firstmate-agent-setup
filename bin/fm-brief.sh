...
4. Report status by appending one line:
   ...
   Declare that wait BEFORE you block on something long you started yourself - a full test run, a
   background round, a review page you are holding - because from outside your pane a worker waiting
   on its own command and a wedged one look identical, and the undeclared case costs firstmate a
   repeated false alarm it can only answer by reading everything again.
